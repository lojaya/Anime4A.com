...
function Drive($link) {
	...
	foreach($data as $list) {
		$data2 = explode('|', $list);
		if($data2[0] == 37) {$js	.= '{label: "1080p", file: "'.preg_replace("/\/[^\/]+\.google\.com/","/redirector.googlevideo.com",$data2[1]).'?title=1080p"},';}
		if($data2[0] == 22) {$js	.= '{label: "720p", file: "'.preg_replace("/\/[^\/]+\.google\.com/","/redirector.googlevideo.com",$data2[1]).'?title=720p"},';}
        if($data2[0] == 59) {$js	.= '{label: "480p", file: "'.preg_replace("/\/[^\/]+\.google\.com/","/redirector.googlevideo.com",$data2[1]).'?title=480p"},';}
		if($data2[0] == 18) {$js	.= '{label: "360p", file: "'.preg_replace("/\/[^\/]+\.google\.com/","/redirector.googlevideo.com",$data2[1]).'?title=360p"},';}
	}
	return rtrim($js, '|');
}
?>