...
class J2T {
    ...
    public function getSource(){
        return $this->_is_plugin_loader->getSource();
    }
...
class J2T_Plugin_Google_Photos extends J2T_Plugin_Abstract {

	/**
	 *
	 * @var string
	 */
	protected $plugin_id = 'photos.google.com';
	/**
	 *
	 * @var int
	 */
	...
	private $_pattern = array(
		'valid_link' => array(
			'/photos.google.com\/share\/[a-zA-Z0-9_-]+/i'
		),
		'json' => array(
			'/{\'preload\':\s*(.+)}},/ism',
			'/feedPreload:\s*(.+)}},/ism'
	...
	private function getMediaContent(){
        $media_content = $this->getPhoto();

        $data = array();
        if(count($media_content))
        {
